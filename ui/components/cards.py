...
{progress_bar_html}
        ...
        with col_actions:
            ...
            with c_edit:
                if st.button("✏️", key=f"edit_{k_id}", help="Edit Metadata", use_container_width=True):
            ...
            with c_del:
                if st.session_state.get('confirm_del') == session_id:
                    if st.button("✔️", key=f"y_{k_id}", use_container_width=True, help="Confirm Delete"):
                        library_service.repository.delete_session(session_id)
                        st.session_state.sessions.pop(session_id, None)
                        st.session_state.pop('confirm_del', None)
                        st.rerun()
                else:
                    if st.button("🗑️", key=f"del_{k_id}", use_container_width=True, help="Remove from Library"):
